...
public class WeeklyTableController extends BaseRequiredAuthentication {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response, Account account)
        ...
        List<Attendance> attendanceTable = new ArrayList<>();
        Student s = (Student) session.getAttribute("stu");
        for (Session at : listSessionByStuid) {
            Attendance a = attDB.getStudentByAttendance(at.getSeid(), s.getStuid());
            
                attendanceTable.add(a);
            

        }

        request.setAttribute("attendanceTable", attendanceTable);
